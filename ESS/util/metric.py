'''IoU'''
import numpy as np
from util.label_constants import *
...
def evaluate_200(pred_ids, gt_ids, stdout=False, dataset='scan_200', logger=None):
    if stdout:
        print('evaluating', gt_ids.size, 'points...')
    if 'scan_200' in dataset:
        ...
        for i in range(N_CLASSES):
            ...
            try:
                if 'matterport' in dataset:
                    ...
                    logger.info('{0:<14s}: {1:>5.3f}   ({2:>6d}/{3:<6d})'.format(
                        label_name,
                        class_ious[label_name][0],
                        class_ious[label_name][1],
                        class_ious[label_name][2]))
...
def evaluate(pred_ids, gt_ids, stdout=False, dataset='scannet_3d', logger=None):
    if stdout:
        print('evaluating', gt_ids.size, 'points...')
