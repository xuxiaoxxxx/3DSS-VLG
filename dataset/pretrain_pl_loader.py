...
class FusedFeatureLoader(Point3DLoader):
    ...
    def __init__(self,
        ...
        # add the path change
        if split == 'train':
            # self.datapath_feat = '/data/xuxiaoxu/code/openvocabulary/ovdet_2d/pointnet/openseg_pretrain_feat_3d'
            self.datapath_feat = '/data/xuxiaoxu/code/openvocabulary/ovdet_2d/pointnet/openseg_pretrain_feat_3d_w_logit' # 带权重的
    ...
    def __getitem__(self, index_long):
        ...
        if 'nuscenes' not in self.dataset_name:
            ...
            if self.split == 'val':
                ...
                processed_data = torch.load(join(
                self.datapath_feat, scene_name+'.pt'))
        else:
            # no repeated file
        ...
        feat_3d, mask_chunk, logit = processed_data['feat'], processed_data['mask_full'], processed_data['logit']
        ...
        return coords, feats, labels, feat_3d, mask, logit
...
def collation_fn(batch):
    ...
    '''
    coords, feats, labels, feat_3d, mask_chunk, logit = list(zip(*batch))

    for i in range(len(coords)):
        coords[i][:, 0] *= i

    return torch.cat(coords), torch.cat(feats), torch.cat(labels), \
        torch.cat(feat_3d), torch.cat(mask_chunk), torch.cat(logit)
