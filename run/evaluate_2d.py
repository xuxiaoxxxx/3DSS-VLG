    ...
    with torch.no_grad():
        ...
        for rep_i in range(args.test_repeats):
            ...
            for i, (coords, feat, label, inds_reverse) in enumerate(tqdm(val_data_loader)):
                ...
                # scene_name = val_data_loader.dataset.data_paths[i].split('/')[-1].split('.pth')[0][:-11]
                # save_path = os.path.join(saved_feature_folder, scene_name + '.npy')
                # print("save_path:", save_path)
                # np.save(save_path, logits_pred)
                # print(a)
                ...
                if vis_gt:
                    ...
                    if 'nuscenes' in labelset_name:
                        ...
                        visualize_labels(list(all_digits), labelset, 
                            palette, os.path.join(save_folder, '{}_label.jpg'.format(save_name)), ncol=all_digits.shape[0])
                # print(a)
                if eval_iou:
                    if mark_no_feature_to_unknown:
